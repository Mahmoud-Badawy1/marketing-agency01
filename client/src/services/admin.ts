...
export function getAdminToken(): string | null {
  ...
  if (expiry && Date.now() > parseInt(expiry, 10)) { clearAdminToken(); return null; }
  return token;
}

export function setAdminToken(token: string, expiresIn?: number): void {
  localStorage.setItem(ADMIN_TOKEN_KEY, token);
  if (expiresIn) localStorage.setItem(ADMIN_TOKEN_EXPIRY_KEY, String(Date.now() + expiresIn));
}
...
export function isAdminLoggedIn(): boolean { return !!getAdminToken(); }
...
export async function adminFetch(url: string, options?: RequestInit): Promise<Response> {
  const token = getAdminToken();
  if (!token) { clearAdminToken(); window.location.href = "/admin/login"; return new Response(JSON.stringify({ message: "غير مصرح" }), { status: 401 }); }
