...
import { AlertTriangle, Sparkles, TrendingUp, CheckCircle2 } from "lucide-react";
...
export default function WhySection() {
  ...
  const displayCards = dynamicCards && dynamicCards.length > 0
    ...
    <section id="why" className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <motion.h2
            variants={fadeInUp}
            className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-foreground tracking-tight leading-[1.25] pb-1 max-w-3xl mx-auto"
          >
            {t("why.title_main")}
            <span className="ms-2 inline-block text-transparent bg-clip-text bg-gradient-to-r from-accent to-blue-500 h-[55px] mt-2">
              {t("why.title_highlight")}
            </span>
          </motion.h2>
          
          <motion.p variants={fadeInUp} className="mt-6 text-muted-foreground max-w-2xl mx-auto text-lg leading-relaxed">
            {t("why.description")}
          </motion.p>
        </motion.div>

        {/* Modern Asymmetrical Auto-Gird / Bento Concept */}
        <motion.div
        ...
        >
          {displayCards.map((card: any, i: number) => {
            ...
            // Dynamic classes to create an asymmetrical "bento" feel if there are exactly 3 cards
            ...
            return (
              <motion.div 
                ...
                variants={fadeInUp} 
                custom={i}
                className={`group relative flex flex-col p-[1px] rounded-[2rem] overflow-hidden ${colSpan}`}
              >
                ...
                <div className="relative h-full bg-card/40 backdrop-blur-2xl p-8 sm:p-10 rounded-[2rem] flex flex-col border border-white/5 dark:border-white/5 z-10 transition-transform duration-500 group-hover:scale-[0.99] shadow-xl hover:shadow-2xl">
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-4 tracking-tight group-hover:text-accent transition-colors duration-300">
                    {t(card.title)}
                  </h3>
                  
                  <p className="text-muted-foreground leading-relaxed flex-grow text-[15px] sm:text-base">
                    {t(card.description)}
                  </p>
