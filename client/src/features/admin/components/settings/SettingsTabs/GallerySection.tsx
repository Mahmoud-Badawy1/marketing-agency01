      ...
      <AccordionContent className="pt-0">
        <CardContent className="space-y-6 pt-6">
            ...
            <div key={i} className="border p-4 rounded-lg bg-card/50 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <BilingualInput
                  ...
                  onChange={(v) =>
                    setGalleryCards((p: any[]) =>
                      p.map((s, idx) => (idx === i ? { ...s, badge: v } : s)),
                    )
                  }
                />
                <BilingualInput
                  label="لون الشارة (Tailwind)"
                  value={{ ar: card.badgeColor, en: card.badgeColor }}
                  onChange={(v) =>
                    setGalleryCards((p: any[]) =>
                      p.map((s, idx) =>
                        idx === i ? { ...s, badgeColor: v.en } : s,
                      ),
                    )
                  }
                />
          ...
          <Button
            ...
            onClick={() => handleSave("gallery_cards", galleryCards)}
            disabled={isSaving}
          >
            حفظ المعرض
          </Button>
