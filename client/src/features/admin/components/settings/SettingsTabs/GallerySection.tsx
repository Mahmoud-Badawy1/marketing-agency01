    ...
    <AccordionItem value="item-8Gallery" className="bg-card border shadow-sm rounded-lg overflow-hidden mb-6 border-none">
      <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-muted/50 transition-colors border-none group">
        <div className="flex items-center gap-2">
          <ImagesIcon className="h-5 w-5 text-primary" />
      ...
      <AccordionContent className="pt-0">
        <CardContent className="space-y-6 pt-6">
          <div className="flex justify-end">
            <Button variant="outline" size="sm" onClick={() => setGalleryCards((p: any[]) => [...p, { badge: { ar: "", en: "" }, title: { ar: "", en: "" }, description: { ar: "", en: "" }, image: "", badgeColor: "bg-primary" }])}>
          ...
          {galleryCards.map((card, i) => (
            <div key={i} className="border p-4 rounded-lg bg-card/50 space-y-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <Button variant="ghost" size="icon" onClick={() => setGalleryCards((p: any[]) => p.filter((_, idx) => idx !== i))}>
              ...
              <ImageUploadInput label="الصورة" value={card.image} onChange={(v: string) => setGalleryCards((p: any[]) => p.map((s, idx) => (idx === i ? { ...s, image: v } : s)))} />
            </div>
          ))}
          <Button className="w-full mt-4" onClick={() => handleSave("gallery_cards", galleryCards)} disabled={isSaving}>
            حفظ المعرض
          </Button>
        </CardContent>
      </AccordionContent>
