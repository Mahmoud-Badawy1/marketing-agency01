      ...
      <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-muted/50 transition-colors border-none group">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold">
            تنبيه حدث القادم (Upcoming Event)
          </span>
        </div>
      </AccordionTrigger>
      <AccordionContent className="pt-0">
        <CardContent className="space-y-6 pt-6">
          <div className="flex items-center space-x-2 space-x-reverse mb-4">
            <Switch
              checked={upcomingEvent.visible}
              onCheckedChange={(v) =>
                setUpcomingEvent((p: any) => ({ ...p, visible: v }))
              }
            />
          ...
          <BilingualInput
            label="شارة الحدث"
            value={upcomingEvent.badge}
            onChange={(v) => setUpcomingEvent((p: any) => ({ ...p, badge: v }))}
          />
          <BilingualInput
            label="اسم الحدث"
            value={upcomingEvent.title}
            onChange={(v) => setUpcomingEvent((p: any) => ({ ...p, title: v }))}
          />
          <BilingualInput
            label="تاريخ الحدث"
            value={upcomingEvent.date}
            onChange={(v) => setUpcomingEvent((p: any) => ({ ...p, date: v }))}
          />
          <BilingualTextarea
            label="وصف الحدث"
            value={upcomingEvent.description}
            onChange={(v) =>
              setUpcomingEvent((p: any) => ({ ...p, description: v }))
            }
          />
          <ImageUploadInput
            label="صورة خلفية الحدث"
            value={upcomingEvent.backgroundImage}
            onChange={(v: string) =>
              setUpcomingEvent((p: any) => ({ ...p, backgroundImage: v }))
            }
          />
          <Button
            className="w-full mt-4"
            onClick={() => handleSave("upcoming_event", upcomingEvent)}
            disabled={isSaving}
          >
            حفظ الحدث
          </Button>
