      ...
      <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-muted/50 transition-colors border-none group">
        <div className="flex-1 text-start">
          <CardHeader className="flex flex-row items-center justify-between p-0">
            <CardTitle>تعديلات قسم تمكين (Empower Section)</CardTitle>
          </CardHeader>
        </div>
      </AccordionTrigger>
      <AccordionContent className="pt-0">
        <CardContent className="space-y-6 pt-6">
          <BilingualInput label="العنوان الفرعي" value={empowerSection.subtitle} onChange={(v: any) => setEmpowerSection((p: any) => ({ ...p, subtitle: v }))} />
          <BilingualInput label="العنوان الرئيسي" value={empowerSection.title} onChange={(v: any) => setEmpowerSection((p: any) => ({ ...p, title: v }))} />
          <BilingualInput label="الجزء المميز من العنوان" value={empowerSection.titleHighlight} onChange={(v: any) => setEmpowerSection((p: any) => ({ ...p, titleHighlight: v }))} />
          <BilingualTextarea label="الوصف" value={empowerSection.description} onChange={(v: any) => setEmpowerSection((p: any) => ({ ...p, description: v }))} />
          <BilingualInput label="نص الزر" value={empowerSection.ctaText} onChange={(v: any) => setEmpowerSection((p: any) => ({ ...p, ctaText: v }))} />
            ...
            {empowerSection.stats?.map((st: any, i: number) => (
              <div key={i} className="flex gap-2 items-start border p-3 rounded-md bg-muted/10">
                <div className="flex-1 space-y-2">
                  ...
                  <BilingualInput label={`وصف ${i + 1}`} value={st.label} onChange={(v: any) => setEmpowerSection((p: any) => { const s = [...p.stats]; s[i].label = v; return { ...p, stats: s }; })} />
                </div>
